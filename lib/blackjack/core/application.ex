...
#   alias Blackjack.{PlayerInfo, Card, User, Repo}
#   alias Blackjack.Web.Controllers.UsersController
...
#     # Enum.map(1..player_count, fn count ->
#     #   if count == 1 do
#     #     User.start_link(Map.put(decoded_user, :player_info, %PlayerInfo{}))
#     #   else
#     #     username = IO.gets("What is player #{count} name.\n") |> String.trim()

#     #     case Repo.get_by!(Blackjack.Web.Models.User, username: username) do
#     #       user -> User.start_link(Map.put(user, :player_info, %PlayerInfo{}))
...
#           %PlayerInfo{player | hand: List.flatten([deal(2) | player.hand])}
#         end)
#       end)
#     end)
#   end
...
#             %PlayerInfo{p | hand: List.flatten([deal(1) | p.hand])}
#           else
#             p
#           end
#         end)
...
#         Enum.map(players, fn p ->
#           if match?(^p, player) do
#             %PlayerInfo{p | hand: List.flatten([deal(0) | p.hand])}
#           else
#             p
#           end
#         end)
...
#       Enum.map(players, fn %PlayerInfo{hand: hand, total: total} = player ->
#         Enum.map_reduce(Enum.reverse(hand), total, fn %Card{value: [value | one]} = card, acc ->
#           calculation = if(is_ace?(card) && acc > 21, do: Enum.at(0)(one), else: value) + acc

#           case acc do
#             acc when acc <= 21 ->
#               {%PlayerInfo{player | total: calculation}, calculation}

#             acc when acc > 21 ->
#               {%PlayerInfo{player | total: calculation, active: false}, calculation}
#           end
#         end)
#       end)
#     end)
...
#     Agent.update(:players_process, fn players ->
#       for(player <- players, do: %PlayerInfo{player | total: 0})
#     end)
#   end
